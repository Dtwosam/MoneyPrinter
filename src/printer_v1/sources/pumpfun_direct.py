"""Fixture-only direct Pump.fun create decoder and continuity owner.
...
REQUEST_CEILINGS = MappingProxyType(
    {SESSION_REQUEST: 1, BACKFILL_REQUEST: 2, TRANSACTION_REQUEST: 16}
)
...
class _Accounting:
    ...
    def consume(self, operation: FixtureOperation) -> None:
        ...
        if self.operation_counts[operation.rpc_operation] >= _OPERATION_CEILINGS[
...
def run_fixture_cycle(
    ...
    *,
    ...
    ordered = sorted(references.values(), key=lambda item: (item.slot, item.signature))
    if len(ordered) > TRANSACTION_DECODE_CEILING:
        ordered = ordered[:TRANSACTION_DECODE_CEILING]
        continuity_fault = True
        rejections.append(Rejection("TRANSACTION_DECODE_CEILING", None, None))

    observations: list[PumpCreateObservation] = []
    for reference in ordered:
        if reference.err is not None or reference.confirmation_status != "finalized":
            code = "FAILED_TRANSACTION" if reference.err is not None else "MISSING_FINALITY"
            rejections.append(Rejection(code, reference.signature, reference.slot))
            continuity_fault = True
            continue
    ...
    if history_unknown or history_unavailable:
        ...
        boundary = (
            CursorBoundary(ordered[-1].slot, ordered[-1].signature)
            if ordered
