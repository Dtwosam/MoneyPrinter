...
STATE_POST_RC_DISCOVERY_MEMORY_CYCLE = "PERSISTENT_DB_POST_RC_DISCOVERY_MEMORY_CYCLE"
...
def bounded_runtime_executed_rows_exist(db_path: Path, counts: dict[str, int | None]) -> bool:
    token_count = counts.get("printer_tokens") or 0
    pair_count = counts.get("printer_pairs") or 0
    if token_count < 1 or pair_count < 1:
        return False
    ...
    if (counts.get("printer_operator_review_items") or 0) < 1:
        return False
    if (counts.get("printer_scheduler_jobs") or 0) != 3:
        return False
    blockers = [
        "printer_paper_positions",
        "printer_paper_trade_events",
    ...
    with connect_read_only(db_path) as connection:
        ...
        phase35_rows = connection.execute(
            """
            SELECT COUNT(*)
            FROM printer_scheduler_jobs
            WHERE status = 'SUCCEEDED'
        ...
        phase36_rows = connection.execute(
            """
            SELECT COUNT(*)
            FROM printer_scheduler_jobs
            WHERE status = 'SUCCEEDED'
              AND job_name = 'phase36_bounded_self_check'
              AND job_kind = 'BACKUP_SOURCE_CHECK'
              AND locked_at IS NULL
              AND lock_owner IS NULL
            """
    ...
    return (
        int(unsafe_decisions) == 0
        and int(unsafe_audits) == 0
        and int(phase35_rows) == 1
        and int(phase36_rows) == 2
        and int(running_rows) == 0
    )


def long_run_paper_validation_rows_exist(db_path: Path, counts: dict[str, int | None]) -> bool:
    token_count = counts.get("printer_tokens") or 0
    pair_count = counts.get("printer_pairs") or 0
    if token_count < 1 or pair_count < 1:
        return False
    ...
    if (counts.get("printer_paper_audit_reports") or 0) < 1:
        return False
    if (counts.get("printer_operator_review_reports") or 0) < 2:
        return False
    if (counts.get("printer_operator_review_items") or 0) < 8:
        return False
    if (counts.get("printer_scheduler_jobs") or 0) != 7:
        return False
    blockers = [
        "printer_paper_positions",
        "printer_paper_trade_events",
    ...
    with connect_read_only(db_path) as connection:
        ...
        phase37_rows = connection.execute(
            """
            SELECT COUNT(*)
            FROM printer_scheduler_jobs
            WHERE status = 'SUCCEEDED'
        ...
        report_payload = report["report_payload_json"] if report else ""
    required_visible_terms = (
        "SOURCE_FAILURES_VISIBLE",
        "DIRTY_MEMORY_BLOCKED",
        "NO_CLEAN_MEMORY_MATCHES",
        "PAPER_VALIDATION_SAFE_BUT_NO_CLEAN_MEMORY",
        "NO_FAKE_PROFIT",
    )
    report_is_visible = all(term in (report_payload or "") for term in required_visible_terms)
    return (
        int(unsafe_decisions) == 0
        and int(unsafe_audits) == 0
        and int(phase35_rows) == 1
        and int(phase36_rows) == 2
        and int(phase37_rows) == 4
        and int(running_rows) == 0
        and report_is_visible
    )
...
def post_rc_discovery_memory_cycle_rows_exist(db_path: Path, counts: dict[str, int | None]) -> bool:
    token_count = counts.get("printer_tokens") or 0
    pair_count = counts.get("printer_pairs") or 0
    if token_count < 1 or pair_count < 1:
        return False
    if token_count > 4 or pair_count > 4:
        return False
    with connect_read_only(db_path) as connection:
        if not table_exists(connection, "printer_discovery_candidates"):
            return False
        discovery_rows = connection.execute("SELECT COUNT(*) FROM printer_discovery_candidates").fetchone()[0]
        if int(discovery_rows) < 1:
            return False
        rc_report = connection.execute(
            """
            SELECT report_payload_json
            FROM printer_operator_review_reports
            WHERE report_title = 'Phase 38 V1 Paper Release Candidate'
        ...
        if rc_report is None or "PAPER_ONLY_RC_SAFE_BUT_NO_CLEAN_MEMORY" not in (rc_report["report_payload_json"] or ""):
            return False
        running_rows = connection.execute(
            "SELECT COUNT(*) FROM printer_scheduler_jobs WHERE status = 'RUNNING' OR locked_at IS NOT NULL OR lock_owner IS NOT NULL"
        ).fetchone()[0]
        ...
        positions = connection.execute("SELECT COUNT(*) FROM printer_paper_positions").fetchone()[0]
        trade_events = connection.execute("SELECT COUNT(*) FROM printer_paper_trade_events").fetchone()[0]
    return int(running_rows) == 0 and int(unsafe_decisions) == 0 and int(positions) == 0 and int(trade_events) == 0
...
def classify_operator_db_state(db_path: str | Path | None = None, project_root: str | Path | None = None) -> str:
    ...
    if post_rc_discovery_memory_cycle_rows_exist(resolved, counts):
        return STATE_POST_RC_DISCOVERY_MEMORY_CYCLE
    ...
    if bounded_runtime_executed_rows_exist(resolved, counts):
        return STATE_BOUNDED_RUNTIME_EXECUTED
