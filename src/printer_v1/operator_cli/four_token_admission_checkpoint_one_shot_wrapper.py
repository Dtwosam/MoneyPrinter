"""One-use application owner for one authorized four-token admission checkpoint.

The child uses the existing operational 4/2/2 composition and stops at the
Cycle-2 admission outcome. It has a distinct command/profile/application
identity and no continuation-window authority.
"""
...
WRAPPER_SCHEMA_VERSION = (
    "PRINTER_V1_FOUR_TOKEN_ADMISSION_CHECKPOINT_ONE_SHOT_WRAPPER_V1"
)
FINAL_AUTHORIZATION_SCHEMA_VERSION = (
    "PRINTER_V1_FOUR_TOKEN_ADMISSION_CHECKPOINT_FINAL_AUTHORIZATION_V1"
)
AUTHORIZED_COMMAND_MODE = (
    FOUR_TOKEN_ADMISSION_CHECKPOINT_AUTHORIZATION_PROFILE.command_mode
)
...
APPLICATION_ROOT = (
    ...
    / "PrinterOperations"
    / "v2-9-8"
    / "four-token-admission-checkpoint-one-shot-applications"
)
...
class FourTokenAdmissionCheckpointOneShotWrapperError(RuntimeError):
    """Fail-closed operational four-token 4/2/2 wrapper/authorization fault."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise FourTokenAdmissionCheckpointOneShotWrapperError(message)
...
def fixture_authorization_document(
    *,
    ...
    authorization_id: str = "FIXTURE_FOUR_TOKEN_ADMISSION_CHECKPOINT_AUTHORIZATION",
    migration_execution_id: str = "FIXTURE_MIGRATION_058",
    verdict: str = (
        "V2_9_8B_FOUR_TOKEN_ADMISSION_CHECKPOINT_FINAL_AUTHORIZATION_PASS"
    ),
...
def _authorization_document(
    *,
    ...
    return {
        ...
        "operational_policy": exact_admission_checkpoint_policy(),
        "authoritative_database": dict(database),
        "prior_authorizations_non_reusable": list(prior_authorizations_non_reusable),
    }


def validate_four_token_admission_checkpoint_authorization_document(
    ...
    try:
        ...
        raise FourTokenAdmissionCheckpointOneShotWrapperError(
...
def _resolve_authorization(
    *,
    ...
    expected_relative = (
        f"{FOUR_TOKEN_ADMISSION_CHECKPOINT_AUTHORIZATION_PROFILE.authorization_package_root}/"
...
def _default_zero_state_gate(
    ...
    **_unused: Any,
    ...
    try:
        return assert_four_token_admission_checkpoint_zero_state(
            db_path=database,
...
def apply_authorization_once(
    *,
    ...
    try:
        ...
        raise FourTokenAdmissionCheckpointOneShotWrapperError(str(exc)) from exc
