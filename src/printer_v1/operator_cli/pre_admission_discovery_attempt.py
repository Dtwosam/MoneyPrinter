...
def cancel_pair_ready_pre_admission_attempt_for_admission_deadline(
    connection: sqlite3.Connection,
    *,
    attempt_id: str,
    campaign_id: str,
    ...
    """Revoke an unconsumed frozen pair only after its admission deadline.

    The frozen pair evidence and its original EXACT_PAIR_FROZEN first cause are
    immutable. A separate append-only ATTEMPT_DISPOSITION event carries the
    deadline terminal cause for campaign reporting.
    """
    exact_id = _required(attempt_id, "attempt_id")
    exact_campaign_id = _required(campaign_id, "campaign_id")
    exact_run_id = _required(campaign_run_id, "campaign_run_id")
    ...
    if (
        ...
        raise PreAdmissionAttemptError("ADMISSION_DEADLINE_TIME_INVALID")
    deadline = deadline_at.astimezone(timezone.utc)
    instant_dt = now.astimezone(timezone.utc)
    if instant_dt < deadline:
        raise PreAdmissionAttemptError("ADMISSION_DEADLINE_NOT_REACHED")
    ...
    if (
        ...
        raise PreAdmissionAttemptError("PAIR_READY_DEADLINE_SHAPE_INVALID")
    if (
        current.campaign_id != exact_campaign_id
        or current.campaign_run_id != exact_run_id
        or current.authoritative_factory_run_id != exact_factory_id
    ):
        raise PreAdmissionAttemptError("ADMISSION_DEADLINE_OWNERSHIP_MISMATCH")

    load_pre_admission_pair(
        connection, attempt_id=exact_id, require_frozen_lane=True
    )
    instant = instant_dt.isoformat()
    cursor = connection.execute(
        """UPDATE printer_pre_admission_discovery_attempts
           SET attempt_state='CANCELLED', updated_at=?
           WHERE attempt_id=?
    ...
    if cursor.rowcount != 1:
        raise PreAdmissionAttemptError("PAIR_READY_DEADLINE_CANCEL_FAILED")
    return load_pre_admission_attempt(connection, attempt_id=exact_id)
