"""Lane X10.10B — TRACK_NORMAL_FIRST_15M Runtime Handler Boundary.

Defines the bounded execution handler for TRACK_NORMAL_FIRST_15M scheduler
jobs. Parallel to e2h_runtime_handler.py (TRACK_FAST_FIRST_15M).

This handler is called by lane_x10_10_normal_15m_runner when a TRACK_NORMAL
token's snapshot step is executed within the bounded 15m runner.
...
HANDLER_JOB_KIND: str = "TRACK_NORMAL_FIRST_15M"
...
E2H_NORMAL_STATUS_EXECUTED: str = "E2H_NORMAL_TRACK_NORMAL_FIRST_15M_EXECUTED"
E2H_NORMAL_STATUS_BLOCKED: str = "E2H_NORMAL_TRACK_NORMAL_FIRST_15M_BLOCKED"
...
def is_handler_registered() -> bool:
    """Return True — TRACK_NORMAL_FIRST_15M handler is registered in Lane X10.10B."""
    return True
...
def validate_token_entry(entry: dict[str, Any]) -> tuple[bool, str]:
    """Validate a single token entry for TRACK_NORMAL_FIRST_15M.
...
def execute_track_normal_first_15m_job(
    connection: sqlite3.Connection,
    ...
    *,
    ...
    """Execute handler for a TRACK_NORMAL_FIRST_15M scheduler job.
    ...
    if not transport_ok:
        return {
            ...
            "status": E2H_NORMAL_STATUS_BLOCKED,
            "executed": False,
    ...
    return {
        "handler": HANDLER_JOB_KIND,
        "status": E2H_NORMAL_STATUS_EXECUTED,
        "executed": True,
