...
_AUTHORIZATION_ID = re.compile(
    r"^V2_9_8B_FOUR_TOKEN_ADMISSION_CHECKPOINT_AUTH_\d{8}T\d{6}Z_[0-9a-f]{8}$"
)
...
class FourTokenAdmissionCheckpointAuthorizationPreparationError(RuntimeError):
    """Fail-closed four-token authorization-preparation fault."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise FourTokenAdmissionCheckpointAuthorizationPreparationError(message)
...
def _canonical_repository_child(root: Path, relative: str) -> Path:
    ...
    try:
        ...
        raise FourTokenAdmissionCheckpointAuthorizationPreparationError(
...
def _validate_authorization_id(value: Any) -> str:
    try:
        ...
        raise FourTokenAdmissionCheckpointAuthorizationPreparationError(str(exc)) from exc
    _require(
        _AUTHORIZATION_ID.fullmatch(authorization_id) is not None,
        "authorization_id does not use the canonical four-token admission-checkpoint form",
    )
...
def _validate_profile_migration(
    *, root: Path, migration_execution_id: str
    ...
    profile = FOUR_TOKEN_ADMISSION_CHECKPOINT_AUTHORIZATION_PROFILE
    try:
...
def _pre_marker_parity(
    *,
    ...
    try:
        ...
        temporary_root = Path(
            tempfile.mkdtemp(prefix="printer-v1-four-token-admission-checkpoint-prep-", dir=str(parent))
        )
...
def prepare_four_token_admission_checkpoint_authorization(
    *,
    ...
    try:
        ...
        raise FourTokenAdmissionCheckpointAuthorizationPreparationError("authorized_at is malformed") from exc
    ...
    document = _authorization_document(
        ...
        verdict="V2_9_8B_FOUR_TOKEN_ADMISSION_CHECKPOINT_FINAL_AUTHORIZATION_PASS",
    ...
    try:
        ...
        raise FourTokenAdmissionCheckpointAuthorizationPreparationError(str(exc)) from exc
    _require(
        validated["operational_policy"] == exact_admission_checkpoint_policy(),
        "derived operational policy did not survive document validation",
    )

    package_parent = _canonical_repository_child(
        root, FOUR_TOKEN_ADMISSION_CHECKPOINT_AUTHORIZATION_PROFILE.authorization_package_root
    )
    ...
    try:
        ...
        just_written = validate_four_token_admission_checkpoint_authorization_document(
        ...
        except FourTokenAdmissionCheckpointAuthorizationPreparationError as cleanup:
            raise FourTokenAdmissionCheckpointAuthorizationPreparationError(
                f"authorization preparation failed ({original}); cleanup blocked ({cleanup})"
            ) from original
        if isinstance(original, FourTokenAdmissionCheckpointAuthorizationPreparationError):
            raise
        raise FourTokenAdmissionCheckpointAuthorizationPreparationError(
...
__all__ = [
    "FourTokenAdmissionCheckpointAuthorizationPreparationError",
    "prepare_four_token_admission_checkpoint_authorization",
]
