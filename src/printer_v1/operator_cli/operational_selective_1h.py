...
def bind_precreated_1h_campaign_window_memory_row(
    connection: sqlite3.Connection,
    *,
    campaign_id: str,
    run_id: str,
    ...
    """Identity-bind one owned WINDOW_1H row without terminalizing it."""
    stamp = now or _utc_now()
    expected_window_id = str(campaign_window_id)
    rows = connection.execute(
        """SELECT window_id,campaign_id,run_id,cycle_id,token_slot_id,
           ...
           WHERE campaign_id=? AND run_id=? AND cycle_id=? AND token_slot_id=?
             AND window_kind=?""",
        (campaign_id, run_id, cycle_id, token_slot_id, WINDOW_1H),
    ).fetchall()
    if not rows:
        raise Selective1hError("precreated 1h campaign window is absent")
    if len(rows) != 1:
        raise Selective1hError("precreated 1h campaign window identity is ambiguous")
    row = rows[0]
    if str(row["window_id"]) != expected_window_id:
        raise Selective1hError("precreated 1h campaign window id mismatch")
    if (
        str(row["campaign_id"]) != str(campaign_id)
        or str(row["run_id"]) != str(run_id)
        or str(row["cycle_id"]) != str(cycle_id)
        ...
        or int(row["token_row_id"]) != int(token_row_id)
        or int(row["pair_row_id"]) != int(pair_row_id)
        or str(row["window_kind"]) != WINDOW_1H
    ):
        raise Selective1hError("precreated 1h campaign window identity mismatch")
    ...
    if (
        memory is None
        or int(memory["id"]) != int(memory_window_row_id)
        or int(memory["token_id"]) != int(token_row_id)
        or int(memory["pair_id"]) != int(pair_row_id)
        or str(memory["window_kind"]) != WINDOW_1H
    ):
        raise Selective1hError("precreated 1h physical memory identity mismatch")
    prior_state = str(row["window_state"])
    prior_cause = row["first_terminal_cause"]
    prior_terminal_at = row["terminal_at"]
    try:
    ...
    if (
        verify is None
        or int(verify["memory_window_row_id"] or -1) != int(memory_window_row_id)
        or str(verify["window_state"]) != prior_state
        or verify["first_terminal_cause"] != prior_cause
        or verify["terminal_at"] != prior_terminal_at
    ):
        raise Selective1hError("precreated 1h campaign window bind readback failed")
    return {
        "window_id": expected_window_id,
        "memory_window_row_id": int(memory_window_row_id),
