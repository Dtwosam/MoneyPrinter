...
Exactly three operator-approved TRACK_FAST Solana tokens. WINDOW_15M only.
Deterministic A/B/C round-robin rotation (tick % 3). No token starvation.
Separate evidence identity per token. No token/pair mixing.
Idempotent replay preserved. All financial locks remain.

This module provides a Lane X4-specific validator and runner path.
Lane X2 (two-token) and Lane X3 (lifecycle) are NOT modified.
The existing single-token E2I / E2J / Lane U path is NOT modified.

Token list shape (Lane X4 format — identical field names to Lane X2):
...
A/B/C rotation: tick % 3 == 0 → slot A, 1 → slot B, 2 → slot C.
Each tick: snapshot for the active slot; if that slot's window is elapsed,
close it and run Lane K.

No starvation guarantee: deterministic round-robin means each of the three
slots gets exactly one tick in every three ticks. With a cycle budget that is
a multiple of 3, each token gets exactly the same number of window closes.

Hard rules:
- Operator approval required.
- Exactly 3 TRACK_FAST tokens (not 1, not 2, not 4, not more).
...
- No X5 five-token expansion (separate future lane).
"""
...
LANE_X4_COMMAND_NAME: str = "printer-run-lane-x4-three-token-cycle"
LANE_X4_STATUS_COMPLETED: str = "LANE_X4_COMPLETED"
LANE_X4_STATUS_BLOCKED: str = "LANE_X4_BLOCKED"
LANE_X4_STATUS_STOPPED: str = "LANE_X4_STOPPED"

# Exactly three tokens — not one, not two, not four.
LANE_X4_EXACT_TOKEN_COUNT: int = 3

# Token list field names (same as Lane X2 format).
...
# Duration profiles — same as Lane X2.
...
# Window-kind policy — same as Lane X2.
...
# Slot labels for the three tokens.
_SLOT_A: str = "A"
_SLOT_B: str = "B"
_SLOT_C: str = "C"

# Step status strings — match E2J constants so downstream consumers can compare.
_X4_STEP_EXECUTED: str = "E2J_FIRST_15M_CYCLE_EXECUTED"
_X4_STEP_BLOCKED: str = "E2J_FIRST_15M_CYCLE_BLOCKED"

_X4_JOB_KIND: str = "TRACK_FAST_FIRST_15M"
_X4_JOB_NAME_PREFIX: str = "x4_track_fast_15m_slot"
_X4_LOCK_OWNER_PREFIX: str = "lane_x4_three_token_slot"
...
# ---------------------------------------------------------------------------
# Lane X4-specific three-token validator
# ---------------------------------------------------------------------------

def _load_and_validate_three_token_list(
    path: str | Path | None,
) -> tuple[bool, str, dict[str, Any], dict[str, Any], dict[str, Any]]:
    """Validate exactly 3 TRACK_FAST approved Solana tokens for Lane X4.

    Returns (valid, reason, token_a_entry, token_b_entry, token_c_entry).

    Uses the Lane X4 token-list format (tracking_lane / operator_approved).
    Does NOT call or modify the X2 or E2I validators.
    """
    if path is None:
    ...
    if len(tokens) != LANE_X4_EXACT_TOKEN_COUNT:
        return (
            False,
            f"Lane X4 requires exactly {LANE_X4_EXACT_TOKEN_COUNT} TRACK_FAST tokens;"
    ...
    if len(set(mints)) != LANE_X4_EXACT_TOKEN_COUNT:
        return (
            False,
            f"duplicate token_mint detected: {mints};"
            " all three tokens must have distinct mints",
            {},
            {},
            {},
        )
    ...
    if len(set(pairs)) != LANE_X4_EXACT_TOKEN_COUNT:
        return (
            False,
            f"duplicate pair_address detected: {pairs};"
            " all three tokens must have distinct pair addresses",
            {},
            {},
            {},
        )

    mint_a, mint_b, mint_c = mints[0], mints[1], mints[2]
    return (
        True,
        f"3 TRACK_FAST approved solana tokens validated:"
...
# ---------------------------------------------------------------------------
# Per-token scheduler job helpers (X4-specific, mirrors X2 pattern)
# ---------------------------------------------------------------------------

def _create_x4_job(connection: sqlite3.Connection, slot: str) -> int:
    """Create a TRACK_FAST_FIRST_15M scheduler job for slot A, B, or C."""
    running = int(
        connection.execute(
    ...
    if running:
        raise ValueError(
            f"Lane X4 slot {slot}: {running} RUNNING job(s) already exist;"
    ...
    cursor = connection.execute(
        ...
        """,
        (f"{_X4_JOB_NAME_PREFIX}_{slot.lower()}", _X4_JOB_KIND, now, now, now),
    )
    return int(cursor.lastrowid)


def _claim_x4_job(connection: sqlite3.Connection, job_id: int, slot: str) -> bool:
    now = _utc_now()
    cursor = connection.execute(
        """
        ...
        WHERE id = ?
          AND status = 'PENDING'
        """,
        (f"{_X4_LOCK_OWNER_PREFIX}_{slot.lower()}", now, now, now, job_id),
    )
    return int(cursor.rowcount) == 1


def _complete_x4_job(connection: sqlite3.Connection, job_id: int) -> None:
    now = _utc_now()
    connection.execute(
        """
        UPDATE printer_scheduler_jobs
...
def _fail_x4_job(connection: sqlite3.Connection, job_id: int, error: str) -> None:
    now = _utc_now()
    connection.execute(
        """
        UPDATE printer_scheduler_jobs
...
# ---------------------------------------------------------------------------
# Per-token step execution (X4-specific — no E2J token-list loading, no E2G gate)
# ---------------------------------------------------------------------------

def _run_x4_token_step(
    db_path: str | Path,
    mint: str,
    slot: str,
    *,
    ...
    """Run one snapshot or window-close step for one token (X4 variant).

    Mirrors the X2 per-token step but uses X4 job helpers and slot labels A/B/C.
    Takes mint directly (no token-list file loading).
    No E2G gate check (X4 validates upfront via _load_and_validate_three_token_list).
    Calls E2H, E2M, E2O, E2Q through the same governed path as X2.
    ...
    try:
        job_id = _create_x4_job(connection, slot)
        connection.commit()

        claimed = _claim_x4_job(connection, job_id, slot)
        if not claimed:
            raise ValueError(
                f"Lane X4 slot {slot}: failed to claim job {job_id}"
            )
        connection.commit()

        job_row = _load_x4_job_row(connection, job_id)
        if job_row is None:
            raise ValueError(
                f"Lane X4 slot {slot}: job {job_id} not found after claim"
            )
        ...
        if handler_result.get("executed", False):
            ...
            if source_response_id is not None:
                ...
                if snapshot_result.get("e2m_status") == _E2M_BLOCKED:
                    ...
                    _fail_x4_job(connection, job_id, reason)
                    ...
                    _complete_x4_job(connection, job_id)
        ...
        if job_id is not None:
            try:
                _fail_x4_job(connection, job_id, str(exc))
                connection.commit()
            except Exception:
                pass
    finally:
    ...
    return {
        "slot": slot,
        "mint": mint,
        "e2j_status": _X4_STEP_EXECUTED if executed else _X4_STEP_BLOCKED,
        "executed": executed,
...
def _blocked_result(
    ...
    return {
        "command": LANE_X4_COMMAND_NAME,
        "lane_x4_status": LANE_X4_STATUS_BLOCKED,
...
# ---------------------------------------------------------------------------
# Main bounded three-token runner
# ---------------------------------------------------------------------------

def run_three_token_memory_factory_cycle(
    ...
    *,
    ...
    """Run a bounded three-token A/B/C rotation Memory Factory cycle for Lane X4.

    Parameters
    ----------
    token_list_path:
        Path to the Lane X4 token list JSON (exactly 3 TRACK_FAST tokens).
    db_path:
    ...
    _cycle_budget:
        Test-only: max total window-close events across all three tokens combined.
        None = duration controls.

    Rotation guarantee
    ------------------
    tick % 3 == 0 → slot A, 1 → slot B, 2 → slot C.
    With _cycle_budget = 3k, each slot gets exactly k window closes.
    No starvation: the round-robin distributes ticks equally across all three slots.
    """
    ...
    if not operator_approved:
        blocked_reasons.append(
            "operator_approved must be True to run the Lane X4 three-token cycle"
        )
    ...
    if window_kind in _FORBIDDEN_AS_MAIN_WINDOW:
        ...
        blocked_reasons.append(
            f"window_kind {window_kind!r} real collection is disabled in Lane X4;"
        ...
        blocked_reasons.append(
            ...
            f" Lane X4 only supports {_ENABLED_MAIN_WINDOW_KIND!r}"
        )
    ...
    # Token list validation — requires exactly 3 TRACK_FAST tokens.
    ...
    if token_list_path is not None and Path(token_list_path).is_file():
        token_list_valid, token_list_reason, token_a_entry, token_b_entry, token_c_entry = (
            _load_and_validate_three_token_list(token_list_path)
        )
    ...
    tick_count: int = 0
    final_status = LANE_X4_STATUS_COMPLETED
    ...
    while True:
        ...
        # A/B/C round-robin: tick % 3 maps to index 0 (A), 1 (B), 2 (C).
        active_idx = tick_count % LANE_X4_EXACT_TOKEN_COUNT
        ...
        snap_result = _run_x4_token_step(
            db_path,
            mint,
            slot,
            adapter=adapter,
        ...
        if snap_result.get("e2j_status") != _X4_STEP_EXECUTED:
            stopped_safely_reason = (
            ...
            final_status = LANE_X4_STATUS_STOPPED
            break
        ...
        if window_elapsed >= window_close_interval_seconds:
            close_result = _run_x4_token_step(
                db_path,
                mint,
                slot,
                adapter=adapter,
            ...
            if close_result.get("e2j_status") != _X4_STEP_EXECUTED:
                stopped_safely_reason = (
            ...
            # Lane K: process all eligible windows (all three tokens).
    ...
    if final_status == LANE_X4_STATUS_COMPLETED and stopped_safely_reason is None:
        stopped_safely_reason = (
    ...
    return {
        ...
        "selected_token_count": LANE_X4_EXACT_TOKEN_COUNT,
        "token_a_mint": mint_a,
        "token_b_mint": mint_b,
        "token_c_mint": mint_c,
