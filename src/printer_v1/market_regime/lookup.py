"""Market regime context lookup helpers."""
...
def market_snapshot_is_valid_for_memory(
    ...
    if DataQualityLabel(snapshot["data_quality_label"]) != DataQualityLabel.CLEAN_DATA:
        return False
    if (
        MarketPayloadQualityLabel(snapshot["market_payload_quality_label"])
...
def find_market_regime_snapshot_before(
    ...
    for row in rows:
        if market_snapshot_is_valid_for_memory(row, target_time, max_age_seconds):
            return row
    return None


def find_market_regime_snapshot_after(
    db_path_or_conn: str | Path | sqlite3.Connection,
    target_time: datetime,
    max_age_seconds: int | None = None,
) -> sqlite3.Row | None:
...
def find_nearest_market_regime_snapshot(
    db_path_or_conn: str | Path | sqlite3.Connection,
    target_time: datetime,
    max_age_seconds: int | None = None,
) -> sqlite3.Row | None:
    before = find_market_regime_snapshot_before(
        db_path_or_conn,
        target_time,
        max_age_seconds,
    )
    after = find_market_regime_snapshot_after(
        db_path_or_conn,
        target_time,
        max_age_seconds,
    )
