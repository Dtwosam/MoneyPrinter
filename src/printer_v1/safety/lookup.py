"""Safety / Rug lookup helpers for future memory and audit use."""
...
def safety_snapshot_blocks_clean_memory(snapshot: sqlite3.Row | dict | None) -> bool:
    if snapshot is None:
        return False
    return (
        SafetyStatusLabel(snapshot["safety_status_label"]) == SafetyStatusLabel.SAFETY_UNSAFE
        or RugRiskLabel(snapshot["rug_risk_label"]) == RugRiskLabel.RUG_RISK_CRITICAL
        or SafetyGateLabel(snapshot["safety_gate_label"])
        in {
            SafetyGateLabel.BLOCK_UNSAFE_CONTEXT,
            SafetyGateLabel.DO_NOT_TRAIN_SAFETY_CONTEXT,
        }
    )


def safety_snapshot_is_valid_for_memory(
    snapshot: sqlite3.Row | dict | None,
    target_time: datetime,
    max_age_seconds: int | None = None,
) -> bool:
    if snapshot is None or safety_snapshot_blocks_clean_memory(snapshot):
        return False
    ...
    if DataQualityLabel(snapshot["data_quality_label"]) != DataQualityLabel.CLEAN_DATA:
        return False
    if (
        SafetyPayloadQualityLabel(snapshot["safety_payload_quality_label"])
...
def find_latest_safety_rug_snapshot(
...
def find_safety_rug_snapshot_before(
    db_path_or_conn: str | Path | sqlite3.Connection,
    token_id: int,
    pair_id: int | None,
    target_time: datetime,
    ...
    for row in rows:
        if safety_snapshot_is_valid_for_memory(row, target_time, max_age_seconds):
            return row
    return None


def find_nearest_safety_rug_snapshot(
    db_path_or_conn: str | Path | sqlite3.Connection,
    token_id: int,
    pair_id: int | None,
    target_time: datetime,
    ...
    before = find_safety_rug_snapshot_before(
        db_path_or_conn,
        token_id,
        pair_id,
        target_time,
