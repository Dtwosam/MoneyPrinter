...
$AuthBranch = 'agent/v2-9-8b-post-dtw94-window15m-authorization-preparation'
$AuthHead = 'b44e7156dfd1979582502190385a0f45f67c41e6'
$Db = Join-Path $Repo 'data/printer_v1.sqlite3'
$PreviousAuth = Join-Path $Repo 'operator-runs/v2-9-8b-window-15m-final-authorization/V2_9_8B_WINDOW_15M_AUTH_20260809T011312Z/final_authorization.json'
$Python = Join-Path $Repo '.venv/bin/python'
$Stamp = (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
$Evidence = Join-Path $HOME "PrinterV1OperatorReadiness/POST_DTW94_WINDOW15M_AUTH_$Stamp"
...
try {
    ...
    if (-not (Test-Path -LiteralPath $PreviousAuth -PathType Leaf)) { Fail 'DTW94 authorization template unavailable' }

    New-Item -ItemType Directory -Path $Evidence -Force | Out-Null
    $env:PRINTER_POST_DTW94_REPO = $Repo
    $env:PRINTER_POST_DTW94_BRANCH = $AuthBranch
    $env:PRINTER_POST_DTW94_HEAD = $AuthHead
    $env:PRINTER_POST_DTW94_DB = $Db
    $env:PRINTER_POST_DTW94_TEMPLATE = $PreviousAuth
    $env:PRINTER_POST_DTW94_EVIDENCE = $Evidence
...
repo = Path(os.environ['PRINTER_POST_DTW94_REPO']).resolve()
branch = os.environ['PRINTER_POST_DTW94_BRANCH']
head = os.environ['PRINTER_POST_DTW94_HEAD']
db = Path(os.environ['PRINTER_POST_DTW94_DB']).resolve()
template_path = Path(os.environ['PRINTER_POST_DTW94_TEMPLATE']).resolve()
evidence = Path(os.environ['PRINTER_POST_DTW94_EVIDENCE']).resolve()

expected_db = {
    'path': str(db),
    'sha256': 'd3c89f3e5f2397c2926fdaffd2588c0239c745b28e63709d565e8eef6c7c36c2',
    'size': 71561216,
    'inode': 1230526,
...
try:
    ...
    if len(history) != 23 or history[-1] != 'V2_9_8B_WINDOW_15M_AUTH_20260809T011312Z':
    ...
    if template.get('schema_version') != 'PRINTER_V1_WINDOW_15M_FINAL_AUTHORIZATION_V2':
        raise RuntimeError('DTW94 template is not canonical V2 authorization schema')
    ...
    document['verdict'] = 'V2_9_8B_POST_DTW94_FRESH_WINDOW_15M_ONE_USE_FINAL_AUTHORIZATION_PASS'
    ...
    result = {
        'status': 'PASS',
        'verdict': 'V2_9_8B_POST_DTW94_FRESH_WINDOW_15M_ONE_USE_AUTHORIZATION_PACKAGE_REVIEW_PASS',
    ...
    print(json.dumps({
        'status': 'BLOCKED',
        'verdict': 'V2_9_8B_POST_DTW94_FRESH_WINDOW_15M_ONE_USE_AUTHORIZATION_PREPARATION_BLOCKED',
        'phase': phase,
        'authorization_id': auth_id,
