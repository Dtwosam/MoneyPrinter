...
def replace_once(path: str, old: str, new: str) -> None:
    ...
    if old not in text:
        raise RuntimeError(f"expected anchor missing in {path}: {old[:140]!r}")
...
# Replace the small pure-policy module with the approved campaign barrier owner.
Path(STANDARD).write_text(r'''"""Operational policy and campaign barrier for standard first-four-hour collection.
...
# Dedicated factory authority is categorical and defaults off.
replace_once(
    FACTORY,
...
# Standard production uses the genuine two-token first-hour path, never proof dispositions.
replace_once(
    FACTORY,
...
# Persist the authority in immutable run configuration for replay/audit.
replace_once(
    FACTORY,
...
# Historical immediate per-token plan remains proof-only.
replace_once(
    FACTORY,
...
# Standard production barrier runs only after the close, campaign binding,
# Scheduler terminalization, and commit are durable.
replace_once(
    FACTORY,
