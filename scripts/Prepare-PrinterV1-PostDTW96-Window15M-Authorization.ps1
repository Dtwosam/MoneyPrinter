...
$AuthBranch = 'agent/v2-9-8b-post-dtw96-window15m-authorization-preparation'
$AuthHead = 'a64d109b043ba86d73b82276fb34ba28561de093'
$Db = Join-Path $Repo 'data/printer_v1.sqlite3'
$PreviousAuth = Join-Path $Repo 'operator-runs/v2-9-8b-window-15m-final-authorization/V2_9_8B_WINDOW_15M_AUTH_20260809T095642Z/final_authorization.json'
$Python = Join-Path $Repo '.venv/bin/python'
$Stamp = (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
$Evidence = Join-Path $HOME "PrinterV1OperatorReadiness/POST_DTW96_WINDOW15M_AUTH_$Stamp"
...
try {
    ...
    if (-not (Test-Path -LiteralPath $PreviousAuth -PathType Leaf)) { Fail 'DTW96 authorization template unavailable' }

    New-Item -ItemType Directory -Path $Evidence -Force | Out-Null
    $env:PRINTER_POST_DTW96_REPO = $Repo
    $env:PRINTER_POST_DTW96_BRANCH = $AuthBranch
    $env:PRINTER_POST_DTW96_HEAD = $AuthHead
    $env:PRINTER_POST_DTW96_DB = $Db
    $env:PRINTER_POST_DTW96_TEMPLATE = $PreviousAuth
    $env:PRINTER_POST_DTW96_EVIDENCE = $Evidence
...
repo = Path(os.environ['PRINTER_POST_DTW96_REPO']).resolve()
branch = os.environ['PRINTER_POST_DTW96_BRANCH']
head = os.environ['PRINTER_POST_DTW96_HEAD']
db = Path(os.environ['PRINTER_POST_DTW96_DB']).resolve()
template_path = Path(os.environ['PRINTER_POST_DTW96_TEMPLATE']).resolve()
evidence = Path(os.environ['PRINTER_POST_DTW96_EVIDENCE']).resolve()
...
try:
    ...
    if db_sidecars():
        raise RuntimeError(f'authoritative DB sidecars present: {db_sidecars()}')
    ...
    if template.get('schema_version') != 'PRINTER_V1_WINDOW_15M_FINAL_AUTHORIZATION_V2':
        raise RuntimeError('DTW96 template is not canonical V2 authorization schema')
    if template.get('authorization_id') != 'V2_9_8B_WINDOW_15M_AUTH_20260809T095642Z':
        raise RuntimeError('DTW96 template identity mismatch')

    phase = 'PACKAGE_BUILD'
    now = datetime.now(timezone.utc).replace(microsecond=0)
    auth_id = f"V2_9_8B_WINDOW_15M_AUTH_{now.strftime('%Y%m%dT%H%M%SZ')}"
    if auth_id in expected_history or auth_id in existing:
    ...
    document['verdict'] = 'V2_9_8B_POST_DTW96_FRESH_WINDOW_15M_ONE_USE_FINAL_AUTHORIZATION_PASS'
    ...
    if db_sidecars():
        raise RuntimeError(f'authoritative DB sidecars appeared during preparation: {db_sidecars()}')
    if app_dir.exists():
        raise RuntimeError('application marker/directory appeared during preparation')

    result = {
        'status': 'PASS',
        'verdict': 'V2_9_8B_POST_DTW96_FRESH_WINDOW_15M_ONE_USE_AUTHORIZATION_PACKAGE_REVIEW_PASS',
    ...
    print(json.dumps({
        'status': 'BLOCKED',
        'verdict': 'V2_9_8B_POST_DTW96_FRESH_WINDOW_15M_ONE_USE_AUTHORIZATION_PREPARATION_BLOCKED',
        'phase': phase,
        'authorization_id': auth_id,
