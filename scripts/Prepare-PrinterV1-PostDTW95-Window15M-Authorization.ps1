...
$AuthBranch = 'agent/v2-9-8b-post-dtw95-window15m-authorization-preparation'
$AuthHead = '00679edb624665d8dc1952ea7d6906324cc1d956'
$Db = Join-Path $Repo 'data/printer_v1.sqlite3'
$PreviousAuth = Join-Path $Repo 'operator-runs/v2-9-8b-window-15m-final-authorization/V2_9_8B_WINDOW_15M_AUTH_20260809T090158Z/final_authorization.json'
$Python = Join-Path $Repo '.venv/bin/python'
$Stamp = (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
$Evidence = Join-Path $HOME "PrinterV1OperatorReadiness/POST_DTW95_WINDOW15M_AUTH_$Stamp"
...
try {
    ...
    if (-not (Test-Path -LiteralPath $PreviousAuth -PathType Leaf)) { Fail 'DTW95 authorization template unavailable' }

    New-Item -ItemType Directory -Path $Evidence -Force | Out-Null
    $env:PRINTER_POST_DTW95_REPO = $Repo
    $env:PRINTER_POST_DTW95_BRANCH = $AuthBranch
    $env:PRINTER_POST_DTW95_HEAD = $AuthHead
    $env:PRINTER_POST_DTW95_DB = $Db
    $env:PRINTER_POST_DTW95_TEMPLATE = $PreviousAuth
    $env:PRINTER_POST_DTW95_EVIDENCE = $Evidence
...
repo = Path(os.environ['PRINTER_POST_DTW95_REPO']).resolve()
branch = os.environ['PRINTER_POST_DTW95_BRANCH']
head = os.environ['PRINTER_POST_DTW95_HEAD']
db = Path(os.environ['PRINTER_POST_DTW95_DB']).resolve()
template_path = Path(os.environ['PRINTER_POST_DTW95_TEMPLATE']).resolve()
evidence = Path(os.environ['PRINTER_POST_DTW95_EVIDENCE']).resolve()

expected_db = {
    'path': str(db),
    'sha256': '59bb25aa71c1283a5086499053409082cb5f411ab4fb2b3e0bebd83da4a960ec',
    'size': 72585216,
    'inode': 1230526,
...
try:
    ...
    if len(history) != 24 or history[-1] != 'V2_9_8B_WINDOW_15M_AUTH_20260809T090158Z':
    ...
    if template.get('schema_version') != 'PRINTER_V1_WINDOW_15M_FINAL_AUTHORIZATION_V2':
        raise RuntimeError('DTW95 template is not canonical V2 authorization schema')
    ...
    document['verdict'] = 'V2_9_8B_POST_DTW95_FRESH_WINDOW_15M_ONE_USE_FINAL_AUTHORIZATION_PASS'
    ...
    result = {
        'status': 'PASS',
        'verdict': 'V2_9_8B_POST_DTW95_FRESH_WINDOW_15M_ONE_USE_AUTHORIZATION_PACKAGE_REVIEW_PASS',
    ...
    print(json.dumps({
        'status': 'BLOCKED',
        'verdict': 'V2_9_8B_POST_DTW95_FRESH_WINDOW_15M_ONE_USE_AUTHORIZATION_PREPARATION_BLOCKED',
        'phase': phase,
        'authorization_id': auth_id,
