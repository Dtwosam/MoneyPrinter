...
class Phase18Point6PersistentLocalDbBootstrapTest(unittest.TestCase):
    ...
    def test_initialize_operator_db_creates_schema_only(self):
        ...
        self.assertEqual(schema["latest_migration"], "021_repeatable_evidence_windows.sql")
        self.assertEqual(len(schema["applied_migrations"]), 21)
        counts = get_core_table_counts(db_path)
        self.assertEqual(counts["printer_schema_migrations"], 21)
        for table in (
            "printer_tokens",
    ...
    def test_explicit_temp_db_path_and_bootstrap_report(self):
        ...
        self.assertEqual(second["schema"]["latest_migration"], "021_repeatable_evidence_windows.sql")
