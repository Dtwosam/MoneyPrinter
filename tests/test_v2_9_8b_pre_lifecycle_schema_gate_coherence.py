...
# --------------------------------------------------------------------------
# 3. Evidence profile: 063 current; 050, 055, 056, 057, 058, 059, 061 and 062 are
#    required immutable historical packages; each keeps its own identity
# --------------------------------------------------------------------------


def test_profile_current_migration_evidence_is_063() -> None:
    profile = git_auth.FOUR_TOKEN_PROOF_AUTHORIZATION_PROFILE
    assert git_auth.MIGRATION_063_PACKAGE_ROOT == (
        "operator-runs/v2-9-8b-migration-063-authorization-preparation"
    )
    assert git_auth.MIGRATION_063_PACKAGE_KIND == "MIGRATION_063_EVIDENCE"
...
def test_profile_declares_050_through_062_as_required_historical_packages() -> None:
    profile = git_auth.FOUR_TOKEN_PROOF_AUTHORIZATION_PROFILE
    roots = {
...
def test_055_through_062_are_promoted_required_historical_packages() -> None:
    """Declaring 055/056/057/058/059/061/062 makes evidence mandatory forever.
    ...
    assert git_auth.MIGRATION_062_PACKAGE_ROOT in declared
    ...
    assert git_auth.MIGRATION_063_PACKAGE_ROOT not in declared
