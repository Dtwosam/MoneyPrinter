...
class Phase30RealMemoryQualityAuditTests(unittest.TestCase):
    ...
    def test_memory_audit_writes_only_audit_report_row(self):
        ...
        self.assertEqual(report["snapshot_gap_summary"]["expected_min_snapshot_count"], 6)
        self.assertTrue(report["snapshot_gap_summary"]["incomplete_15m_window"])
        self.assertEqual(report["source_quality_summary"]["status"], "SOURCE_QUALITY_ACCEPTABLE_WITH_HISTORICAL_FAILURES_VISIBLE")
        self.assertTrue(report["source_quality_summary"]["historical_source_failures_visible"])
        self.assertFalse(report["source_quality_summary"]["required_evidence_failed_or_missing"])
        self.assertEqual(report["context_quality_summary"]["status"], "CONTEXT_BLOCKED_FOR_WINDOW")
