"""
Post-Lane 10 Lane I-B — Bounded 12h Staged Memory Factory Boundary

Thirteen boundary proofs:
1.  operator_approved is required
2.  WINDOW_12H is the only accepted target
3.  WINDOW_24H is explicitly blocked (stage not yet active)
4.  WINDOW_15M, WINDOW_1H, WINDOW_4H, and WINDOW_5M_MICRO_EVENT cannot satisfy 12h
5.  evidence must span >= 720 real minutes
6.  fake 15m/1h/4h composite evidence is blocked
7.  incomplete/dirty/stale/missing data → DIRTY or AUDIT_ONLY, not clean
8.  CLEAN_CANDIDATE only when full 12h evidence gates pass
...
13. no existing Lane H, Lane I-A, 15m, 1h, 4h, or 5m behavior is weakened
"""
...
from printer_v1.operator_cli.lane_i_12h_staged_memory_factory import (
    LANE_IB_MIN_SNAPSHOT_COUNT,
    LANE_IB_MIN_WINDOW_MINUTES,
    LANE_IB_STATUS_AUDIT_ONLY,
    LANE_IB_STATUS_BLOCKED,
    LANE_IB_STATUS_CLEAN_CANDIDATE,
    LANE_IB_STATUS_DIRTY,
    LANE_IB_WINDOW_KIND,
    _HARD_LOCKS,
    _LOCKED_STATE,
    build_12h_evidence_fixture,
    classify_12h_memory_attempt,
)


def _run(evidence, *, operator_approved=True):
    return classify_12h_memory_attempt(evidence, operator_approved=operator_approved)


def _valid():
    return build_12h_evidence_fixture()
...
class LaneIBApprovalTests(unittest.TestCase):
    def test_blocked_without_approval(self):
        r = _run(_valid(), operator_approved=False)
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
    ...
    def test_evidence_none_blocked(self):
        r = classify_12h_memory_attempt(None, operator_approved=True)
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)

    def test_evidence_non_dict_blocked(self):
        r = classify_12h_memory_attempt("not-a-dict", operator_approved=True)  # type: ignore[arg-type]
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
...
# ============================================================
# Proof 2 — WINDOW_12H is the only accepted target
# ============================================================

class LaneIBWindowKindTests(unittest.TestCase):
    def test_window_12h_passes_kind_gate(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_12H"))
        self.assertNotEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)

    def test_window_kind_target_always_12h(self):
        for kind in ("WINDOW_15M", "WINDOW_1H", "WINDOW_4H", "WINDOW_24H", None):
            with self.subTest(kind=kind):
                r = _run(build_12h_evidence_fixture(window_kind=kind))
                self.assertEqual(r["window_kind_target"], LANE_IB_WINDOW_KIND)

    def test_none_window_kind_is_blocked(self):
        r = _run(build_12h_evidence_fixture(window_kind=None))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)

    def test_blocked_reason_mentions_12h(self):
        r = _run(build_12h_evidence_fixture(window_kind=None))
        self.assertTrue(
            any("WINDOW_12H" in reason for reason in r["rejection_reasons"])
        )


# ============================================================
# Proof 3 — WINDOW_24H explicitly blocked (stage not yet active)
# ============================================================

class LaneIBFutureStageBlockTests(unittest.TestCase):
    def test_window_24h_is_explicitly_blocked(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_24H"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
    ...
    def test_window_24h_blocked_reason_says_ordered_activation(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_24H"))
        reasons_text = " ".join(r["rejection_reasons"])
        self.assertIn("24h", reasons_text)
    ...
    def test_24h_explicitly_blocked_not_misclassified(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_24H"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
        self.assertNotEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)
        self.assertNotEqual(r["lane_ib_status"], LANE_IB_STATUS_AUDIT_ONLY)


# ============================================================
# Proof 4 — WINDOW_15M, WINDOW_1H, WINDOW_4H, WINDOW_5M cannot satisfy 12h
# ============================================================

class LaneIBShortWindowRejectionTests(unittest.TestCase):
    def test_window_15m_is_blocked(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_15M"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)

    def test_window_1h_is_blocked(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_1H"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)

    def test_window_4h_is_blocked(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_4H"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)

    def test_window_5m_micro_event_is_blocked(self):
        r = _run(build_12h_evidence_fixture(
            window_kind="WINDOW_5M_MICRO_EVENT",
            is_5m_window=True,
        ))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)

    def test_15m_blocked_reason_mentions_12h(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_15M"))
        self.assertTrue(any("WINDOW_12H" in reason for reason in r["rejection_reasons"]))

    def test_1h_blocked_reason_mentions_12h(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_1H"))
        self.assertTrue(any("WINDOW_12H" in reason for reason in r["rejection_reasons"]))

    def test_4h_blocked_reason_mentions_12h(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_4H"))
        self.assertTrue(any("WINDOW_12H" in reason for reason in r["rejection_reasons"]))
    ...
    def test_4h_is_not_clean_candidate(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_4H"))
        self.assertFalse(r["is_clean_candidate"])


# ============================================================
# Proof 5 — evidence must span >= 720 real minutes
# ============================================================

class LaneIBDurationTests(unittest.TestCase):
    def test_exactly_720m_passes_duration_gate(self):
        r = _run(build_12h_evidence_fixture(window_duration_minutes=720))
        self.assertNotEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_719m_fails_duration_gate(self):
        r = _run(build_12h_evidence_fixture(window_duration_minutes=719))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_480m_fails_duration_gate(self):
        r = _run(build_12h_evidence_fixture(window_duration_minutes=480))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_240m_fails_duration_gate(self):
        # 4h evidence is not sufficient for 12h
        r = _run(build_12h_evidence_fixture(window_duration_minutes=240))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_60m_fails_duration_gate(self):
        # 1h evidence is not sufficient for 12h
        r = _run(build_12h_evidence_fixture(window_duration_minutes=60))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_duration_dirty_reason_mentions_minimum(self):
        r = _run(build_12h_evidence_fixture(window_duration_minutes=100))
        self.assertTrue(
            any("720" in reason for reason in r["rejection_reasons"]),
            msg=f"expected 720 minimum mentioned in {r['rejection_reasons']}",
        )

    def test_min_window_minutes_constant_is_720(self):
        self.assertEqual(LANE_IB_MIN_WINDOW_MINUTES, 720)

    def test_missing_open_snapshot_is_dirty(self):
        r = _run(build_12h_evidence_fixture(has_open_snapshot=False))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_missing_close_snapshot_is_dirty(self):
        r = _run(build_12h_evidence_fixture(has_close_snapshot=False))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)


# ============================================================
# Proof 6 — fake 15m/1h/4h composite evidence is blocked
# ============================================================

class LaneIBFakeCompositeTests(unittest.TestCase):
    def test_fake_15m_composite_is_blocked(self):
        r = _run(build_12h_evidence_fixture(is_fake_15m_composite=True))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
    ...
    def test_fake_1h_composite_is_blocked(self):
        r = _run(build_12h_evidence_fixture(is_fake_1h_composite=True))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
    ...
    def test_fake_4h_composite_is_blocked(self):
        r = _run(build_12h_evidence_fixture(is_fake_4h_composite=True))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
    ...
    def test_no_episode_from_fake_4h_composite(self):
        r = _run(build_12h_evidence_fixture(is_fake_4h_composite=True))
        self.assertFalse(r["is_clean_candidate"])
...
class LaneIBDirtyAuditClassificationTests(unittest.TestCase):
    def test_dirty_data_label_yields_dirty(self):
        r = _run(build_12h_evidence_fixture(data_quality_label="DIRTY_DATA"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_stale_data_label_yields_dirty(self):
        r = _run(build_12h_evidence_fixture(data_quality_label="STALE_DATA"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_missing_critical_data_yields_dirty(self):
        r = _run(build_12h_evidence_fixture(data_quality_label="MISSING_CRITICAL_DATA"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_conflicting_data_yields_dirty(self):
        r = _run(build_12h_evidence_fixture(data_quality_label="CONFLICTING_DATA"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_do_not_train_yields_dirty(self):
        r = _run(build_12h_evidence_fixture(do_not_train=True))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_failed_source_yields_dirty(self):
        r = _run(build_12h_evidence_fixture(source_status="FAILED"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)

    def test_too_few_snapshots_yields_dirty(self):
        r = _run(build_12h_evidence_fixture(snapshot_count=1))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)
    ...
    def test_acceptable_partial_data_yields_audit_only(self):
        r = _run(build_12h_evidence_fixture(data_quality_label="ACCEPTABLE_PARTIAL_DATA"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_AUDIT_ONLY)

    def test_partial_source_status_yields_audit_only(self):
        r = _run(build_12h_evidence_fixture(source_status="PARTIAL"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_AUDIT_ONLY)

    def test_incomplete_context_yields_audit_only(self):
        r = _run(build_12h_evidence_fixture(context_complete=False))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_AUDIT_ONLY)
...
class LaneIBCleanCandidateTests(unittest.TestCase):
    def test_valid_fixture_is_clean_candidate(self):
        r = _run(_valid())
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_CLEAN_CANDIDATE)
    ...
    def test_clean_requires_complete_source(self):
        r = _run(build_12h_evidence_fixture(source_status="PARTIAL"))
        self.assertNotEqual(r["lane_ib_status"], LANE_IB_STATUS_CLEAN_CANDIDATE)
    ...
    def test_clean_requires_full_12h_duration(self):
        r = _run(build_12h_evidence_fixture(window_duration_minutes=480))
        self.assertNotEqual(r["lane_ib_status"], LANE_IB_STATUS_CLEAN_CANDIDATE)
...
class LaneIBZeroCleanTests(unittest.TestCase):
    def test_all_dirty_evidence_yields_zero_clean(self):
        results = [
            _run(build_12h_evidence_fixture(data_quality_label="DIRTY_DATA")),
            _run(build_12h_evidence_fixture(source_status="FAILED")),
            _run(build_12h_evidence_fixture(window_duration_minutes=240)),
        ]
        self.assertEqual(sum(1 for r in results if r["is_clean_candidate"]), 0)

    def test_all_audit_only_yields_zero_clean(self):
        results = [
            _run(build_12h_evidence_fixture(context_complete=False)),
            _run(build_12h_evidence_fixture(source_status="PARTIAL")),
        ]
        self.assertEqual(sum(1 for r in results if r["is_clean_candidate"]), 0)
    ...
    def test_blocked_evidence_yields_zero_clean(self):
        results = [
            ...
            _run(build_12h_evidence_fixture(window_kind="WINDOW_24H")),
            _run(build_12h_evidence_fixture(is_fake_4h_composite=True)),
        ]
        self.assertEqual(sum(1 for r in results if r["is_clean_candidate"]), 0)

    def test_zero_clean_does_not_raise(self):
        r = _run(build_12h_evidence_fixture(data_quality_label="DIRTY_DATA"))
        self.assertIn("lane_ib_status", r)
        self.assertFalse(r["is_clean_candidate"])
...
class LaneIB5mSupportOnlyTests(unittest.TestCase):
    def test_5m_window_flag_is_blocked(self):
        r = _run(build_12h_evidence_fixture(is_5m_window=True))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
...
class LaneIBLockedStateTests(unittest.TestCase):
    ...
    def test_retrieval_never_activated(self):
        for r in self._all_statuses():
            with self.subTest(status=r["lane_ib_status"]):
                self.assertFalse(r["retrieval_activated"])
...
class LaneIBSchedulerGovernorTests(unittest.TestCase):
...
# ============================================================
# Proof 13 — no existing Lane H, Lane I-A, 15m, 1h, 4h, or 5m behavior weakened
#   (structural: Lane I-B is pure dict-in/dict-out,
#    imports no Lane H / Lane I-A / 15m modules, writes no DB rows)
# ============================================================

class LaneIBBehaviorPreservationTests(unittest.TestCase):
    def test_lane_ib_module_does_not_import_lane_h_or_ia(self):
        import printer_v1.operator_cli.lane_i_12h_staged_memory_factory as lib
        with open(lib.__file__, encoding="utf-8") as fh:
            source = fh.read()
        forbidden = [
            "lane_h_", "lane_i_4h_", "e2j_", "e2o_", "e2q_", "e2t_",
            "e2x_", "e2y_", "e2z_",
        ]
        for name in forbidden:
            self.assertNotIn(name, source, f"Lane I-B must not import {name}")

    def test_lane_ib_window_kind_is_12h_not_4h_or_1h_or_15m(self):
        self.assertEqual(LANE_IB_WINDOW_KIND, "WINDOW_12H")
        self.assertNotEqual(LANE_IB_WINDOW_KIND, "WINDOW_4H")
        self.assertNotEqual(LANE_IB_WINDOW_KIND, "WINDOW_1H")
        self.assertNotEqual(LANE_IB_WINDOW_KIND, "WINDOW_15M")

    def test_lane_ib_classify_is_stateless(self):
        r1 = _run(_valid())
        r2 = _run(_valid())
        self.assertEqual(r1["lane_ib_status"], r2["lane_ib_status"])
    ...
    def test_4h_explicitly_blocked_not_misclassified(self):
        r = _run(build_12h_evidence_fixture(window_kind="WINDOW_4H"))
        self.assertEqual(r["lane_ib_status"], LANE_IB_STATUS_BLOCKED)
        self.assertNotEqual(r["lane_ib_status"], LANE_IB_STATUS_DIRTY)
        self.assertNotEqual(r["lane_ib_status"], LANE_IB_STATUS_AUDIT_ONLY)
    ...
    def test_min_window_minutes_is_720(self):
        self.assertEqual(LANE_IB_MIN_WINDOW_MINUTES, 720)

    def test_min_snapshot_count_is_2(self):
        self.assertEqual(LANE_IB_MIN_SNAPSHOT_COUNT, 2)
