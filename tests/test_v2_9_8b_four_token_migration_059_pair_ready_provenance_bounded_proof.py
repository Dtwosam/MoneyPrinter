...
class FullShapePositiveProofTests(unittest.TestCase):
    def test_full_64_file_evidence_shape_passes_canonical_pre_marker_validation(self) -> None:
        ...
        try:
            ...
            self.assertEqual(len(historical_migration), 54)
            self.assertEqual(len(historical_reconciliation), 12)
            self.assertEqual(prepared.file_count, 69)
            self.assertEqual(
            ...
            self.assertTrue(
                any(
                    item["package_kind"] == git_auth.MIGRATION_063_PACKAGE_KIND
...
class ProfileScopeProofTests(unittest.TestCase):
    def test_only_four_token_profiles_receive_063_history_and_pair_ready(self) -> None:
