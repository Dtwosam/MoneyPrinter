-- V2-9.7E.47 B2: persist the moderate-continuation held-to-15m result.
--
-- A measured held-to-15m price change between +5% and +25% previously produced
-- HELD_TO_15M_UNKNOWN purely because it sat between two categorical thresholds.
-- The classifier now returns HELD_TO_15M_MODERATE_CONTINUATION for that band, so
-- the persistence CHECK on printer_micro_events must accept it.
--
-- SQLite cannot alter a CHECK constraint in place, so the table is rebuilt with
-- the identical column set, defaults, foreign keys and indexes; only the
-- held_to_15m_result_label CHECK list gains one categorical value. No row is
-- rewritten, no historical label is reinterpreted, and no new column, score,
-- rank, confidence or weight is introduced. Nothing references
-- printer_micro_events by foreign key, so the rebuild is reference-safe.

CREATE TABLE printer_micro_events_v2 (
    ...
    held_to_15m_result_label TEXT NOT NULL CHECK (held_to_15m_result_label IN ('HELD_TO_15M_CONTINUED', 'HELD_TO_15M_MODERATE_CONTINUATION', 'HELD_TO_15M_CONSOLIDATED', 'HELD_TO_15M_FADED', 'HELD_TO_15M_DUMPED', 'HELD_TO_15M_DEAD', 'HELD_TO_15M_UNKNOWN')),
...
INSERT INTO printer_micro_events_v2 SELECT * FROM printer_micro_events;

DROP TABLE printer_micro_events;

ALTER TABLE printer_micro_events_v2 RENAME TO printer_micro_events;
